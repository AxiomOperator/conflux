...
export async function GET() {
  ...
  const upstream = await fetch(`${API_BASE}/v1/backup/full`, {
    headers: {
  ...
  if (!upstream.ok) {
    return NextResponse.json(
      { error: await readUpstreamError(upstream, "Full backup failed") },
  ...
  const contentDisposition =
    ...
    `attachment; filename="conflux-full-backup-${new Date().toISOString().slice(0, 10)}.zip"`;
